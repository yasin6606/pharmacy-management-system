...
export default function CreditsPage() {
    ...
    const fetchCredits = async () => {
        setLoading(true);
        const params: Record<string, string | number> = {page: 1, limit: 200, paymentMethod: 'credit'};
        if (branchId) params.branchId = branchId;
        const data = await get<PaginatedResponse<Sale>>('/sales', {params});
        if (data) setAllCredits(data.items);
        setLoading(false);
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    <CardTitle>{t('allCredits')}</CardTitle>
                </CardHeader>
                <CardContent>
                    {loading ? (
                            ...
                            <div className="space-y-2">
                                {paginatedGroups.map((group) => (
                                    <Card key={group.basketId} className="border-2">
                                        ...
                                        >
                                            <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs sm:text-sm">
                                            ...
                                            <div className="flex items-center gap-4 ml-4">
                                                {group.isPaid && <CheckCircle className="h-4 w-4 text-green-500" />}
                                                <Badge variant={group.isPaid ? 'success' : 'danger'} className="text-xs">
                                                    {group.isPaid ? t('paid') : formatIRR(group.total, locale)}
                                                </Badge>
